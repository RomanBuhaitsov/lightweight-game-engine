...
Entity *makeEntityFromIndex(int index, int pos_x, int pos_y,
                            const std::optional<SDL_Event> &ev,
  ...
  if (index == 0) {
    ...
    newTile->addComponent(new PhysicsComponent(
        PhysicsComponent::TileBody(ev.value().button.x / 32 * 32 - 1,
                                   ev.value().button.y / 32 * 32, 32, 32, 0.8f),
        message_bus));
    newTile->addComponent(new SpriteComponent(
        message_bus, texture_manager["grasstile.png"], 32, 32, 64, 64));
    ...
    // newSlopeTile->addComponent(new
    // PhysicsComponent(PhysicsComponent::TileBody(ev.button.x / 32 * 32 - 1,
    // ev.button.y / 32 * 32 - 1, 32, 32, 0.8f)));
    newSlopeTile->addComponent(
        new PhysicsComponent(PhysicsComponent::RightTriangleTileBody(
                                 ev.value().button.x / 32 * 32 - 1,
                                 ev.value().button.y / 32 * 32, 32, 32, 0.8f),
                             message_bus));
    newSlopeTile->addComponent(
        new SpriteComponent(message_bus, texture_manager["grass_slope_b.png"],
                            32, 32, 64, 64, 0, -16));
    ...
    dynamicBox->addComponent(new PhysicsComponent(
        PhysicsComponent::TileBody(ev.value().button.x / 32 * 32 - 1,
                                   ev.value().button.y / 32 * 32, 32, 32, 0.8f,
                                   0.1f, true),
        message_bus));
    dynamicBox->addComponent(new SpriteComponent(
        message_bus, texture_manager["wooden_box.png"], 32, 32, 32, 32, 0, 0));
    ...
    coinEntity->addComponent(new PhysicsComponent(
        PhysicsComponent::CharacterBody(ev.value().button.x / 32 * 32 - 1,
                                        ev.value().button.y / 32 * 32, 32 / 2,
                                        32 / 2, 0.1f),
        [](Entity *other) -> bool {
        ...
        message_bus));
    coinEntity->addComponent(
        new SpriteComponent(message_bus, texture_manager["coin.png"], 32 / 2,
                            32 / 2, 32, 32, {4}, 100, 32 / 4, 0));
    coinEntity->addComponent(new SpritePhysicsHandler(message_bus));
    return coinEntity;
  } else if (index == 4) {
    Entity *player = new Entity(EntityType::ET_PLAYER);
    player->addComponent(new PhysicsComponent(
        PhysicsComponent::CharacterBody(0, 0, 24, 24, 0.1f), message_bus));
    player->addComponent(new SpriteComponent(message_bus,
                                             texture_manager["crudewalk.png"],
                                             24, 28, 24, 28, {6, 6}, 100, 4));
    player->addComponent(
        new SpritePhysicsHandler(message_bus, PlayerMovementHandler));
    return player;
  } else if (index == 5) {
    Entity *tile = new Entity(EntityType::ET_GENERIC);
    tile->addComponent(new PhysicsComponent(
        PhysicsComponent::TileBody(pos_x, pos_y, 32, 32, 0.8f), message_bus));
    tile->addComponent(new SpriteComponent(
        message_bus, texture_manager["grasstile.png"], 32, 32, 64, 64, 0, 0));
...
void GameWindow::onNotify(Message message) {
  ...
  case MessageEvent::LMB_PRESSED: {
    Entity *gent = makeEntityFromIndex(
        2, -1, -1, std::nullopt, this->texture_manager, this->message_bus);
...
void GameWindow::init() {
  ...
  int tileIndex = 0;
  Entity *player = makeEntityFromIndex(
      4, -1, -1, std::nullopt, this->texture_manager, this->message_bus);
  ...
  for (int i = 0; i < 28; ++i) {
    int pos_x = i * 32 - 1;
    int pos_y = 448;
    this->entities.insert(makeEntityFromIndex(5, pos_x, pos_y, std::nullopt,

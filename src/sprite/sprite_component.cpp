...
SpriteComponent::SpriteComponent(MessageBus *message_bus, SDL_Texture *texture,
                                 ...
                                 Uint16 animationSpeed, int shift_x,
                                 int shift_y)
    : Component(ComponentType::CT_SPRITE, message_bus), texture(texture),
      src({0, 0, frameWidth, frameHeight}),
      dst({0, 0, spriteWidth, spriteHeight}),
      center({frameWidth / 2, frameHeight / 2}), animationSpeed(animationSpeed),
      currentFrame(0), currentRow(0), lastUpdate(0), nRows(numFrames.size()),
      flip(SDL_FLIP_NONE), playing(true), angle(0.0), shift_x(shift_x),
      shift_y(shift_y) {
  if (!nRows) {
...
SpriteComponent::SpriteComponent(MessageBus *message_bus, SDL_Texture *texture,
                                 int spriteWidth, int spriteHeight,
                                 int frameWidth, int frameHeight, int shift_x,
                                 int shift_y)
    : SpriteComponent(message_bus, texture, spriteWidth, spriteHeight,
                      frameWidth, frameHeight, {1}, 0, shift_x, shift_y) {}

void SpriteComponent::init() { dst.x = dst.y = 0; }

void SpriteComponent::flipVertically() { flip ^= SDL_FLIP_VERTICAL; }

void SpriteComponent::flipHorizontally() { flip ^= SDL_FLIP_HORIZONTAL; }

int SpriteComponent::getSpriteWidth() { return dst.w; }

int SpriteComponent::getSpriteHeight() { return dst.h; }

Uint16 SpriteComponent::getFlipped() { return flip; }

void SpriteComponent::startAnimation() { playing = true; }

void SpriteComponent::stopAnimation() { playing = false; }
...
void SpriteComponent::draw() {
  GAME->renderTexture(texture, src, dst, angle, &center,
